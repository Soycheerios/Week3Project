...
function greaterThan100() {
    ...
    sumOfNumbers += numbers[i];                //sum of numbers is total of numbers array.
}                                                   //create for loop to add numbers array to greaterThan100
{
    ...
    console.log(`Answer 9: ${false}`);    //else print false
}
}
}
...
var monthlyExpenses = ["Groceries", "electricity", "rent", "hobby", "gymMembership"]; //made an array showing necessary monthly cost.
var costOfExpenses = [125.00, 200.00, 1500.00, 150.00, 100.00]; //made array cost of each needs. 
 

function monthlyPayment(weeklySalary, costOfExpenses) {
...
monthlyExpenses.push("hospital bill", "pet Food", "dinner with friends");                      //extra cost
costOfExpenses.push(200.00, 200.00, 100.00);              //Additional cost of expenses

for (i = 0; i < costOfExpenses.length; i++) {
    sumCost += costOfExpenses[i];
}
...
if (sumSalary >= sumCost) {  
    ...
    console.log(`Answer 13: I made enough money for ${monthlyExpenses} with my salary. I will have ${remainder} dollars left in your account!`);
} else {
    console.log(`Answer 13:I can't afford ${monthlyExpenses} with my salary. My balance will be ${remainder}. You need to cut cost somewhere!`);
}
}
}

monthlyPayment(weeklySalary, costOfExpenses);
